...
def tanhKernel(x,y,k=1,delta=0.1):
    np.transpose(x)
    a = np.dot(k,x)
    a = np.dot(a,y)
    a = a - delta
...
def indicator(ind,new_data,m):
    ...
    for i in range(ind.shape[0]):
        ...
        if(m==4):
            ans += tanhKernel(new_data, ind[i][[0,1]])*ind[i][2]*ind[i][3]
    return ans
...
def create_model(data,m):
    ...
    if(m==4):
        for i in range(n):
            for j in range(n):
                P[i][j] = data[i][2]*data[j][2]*tanhKernel(data[i][[0,1]], data[j][[0,1]])
...
def run():
    ...
    for i in range(5):
        ...
        plot_data(f,i)
    axarr[0, 0].set_title('Linear, C=' + str(realC))
    axarr[0, 1].set_title('2nd order Poly, C=' + str(realC))
    axarr[1, 0].set_title('3rd order Poly, C=' + str(realC))
    axarr[1, 1].set_title('Gaussian, C=' + str(realC))
    axarr[2, 1].set_title('tanh, C=' + str(realC))
    pylab.show()
    pylab.savefig("HyperDerp" + str(realC) + ".png")
